...
public class PhotoView extends ImageView {
	...
	private boolean isImageCenterHeight() {
		return Math.round(mImgRect.top) == (mWidgetRect.height() - mImgRect
				.height()) / 2;
	}

	private boolean isImageCenterWidth() {
		return Math.round(mImgRect.left) == (mWidgetRect.width() - mImgRect
				.width()) / 2;
	}
	...
	private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
		...
		public boolean onDoubleTap(MotionEvent e) {
			...
			if (mScale == 1) {
				...
				mTranslate.withTranslate(mTranslateX, mTranslateY,
	...
	private class Transform implements Runnable {
		...
		void withClip(float fromX, float fromY, float deltaX, float deltaY,
				int d) {
			mClipScroll.startScroll((int) (fromX * 10000),
		...
		public void run() {
			...
			if (mClipScroll.computeScrollOffset() || mClip != null) {
				...
				mTmpMatrix.setScale(sx, sy,
						...
						(mImgRect.top + mImgRect.bottom) / 2);
			...
			if (!endAnima) {
				mAnimaMatrix.reset();
				mAnimaMatrix.postScale(mScale, mScale, mDoubleTab.x,
						mDoubleTab.y);
	...
	public void animaFrom(Info info) {
		if (isInit) {
			...
			mTranslate.withTranslate(mTranslateX, mTranslateY, (int) -tx,
					(int) -ty);
	...
	public void animaTo(Info info, Runnable completeCallBack) {
		if (isInit) {
			...
			int tx = (int) (info.mLocalRect.left - mine.mLocalRect.left
					+ info.mImgRect.left - mValues[Matrix.MTRANS_X] * scale);
			int ty = (int) (info.mLocalRect.top - mine.mLocalRect.top
					+ info.mImgRect.top - mValues[Matrix.MTRANS_Y] * scale);

			mTranslate.withScale(mScale, scale);
			mTranslate.withTranslate(mTranslateX, mTranslateY, -mTranslateX
					+ tx, -mTranslateY + ty);
